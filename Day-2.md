...
- アプリケーションは SWA CLI で認証をエミュレーションできる
- DB は開発用 Azure 環境上に VNET なしで構築
  - DB はパブリックネットワークで公開されるが、キー認証はされる (Entra ID 認証にしておくとよりセキュア)
...
- Cosmos DB を使う場合の注意点
  ...
  - パーティションキーは ID に設定 (集計系は RDB に寄せるため、最も分散しやすい設計に)
